...
int32_t serial_unix_set_speed(int32_t fd, struct termios *ti, int32_t speed)
{
...
int
serial_unix_init(serial_if_cfg_t * p_cfg)
{
  struct termios ti;
  ...
  /* Set actual baudrate */
  if (serial_unix_set_speed(fd, &ti, p_cfg->baudrate) < 0)
  {
...
uint32_t
serial_unix_bytes_available(void)
{
    int32_t bytes_avail;
